/*
  ...
  This version modifies TMrandom.c to count the lower number of instructions producing a string
...
int run_step(turing_machine *m){
  ...
  //  show_data(m);
    ...
    // sets the transition as used
    m->transition_table[m->state.control_state][s1].used = 1;
...
turing_machine init_turing_machine(int states, int colors, int blank, D dF, D dR){
  ...
  m.transition_table[0][0].used = 0; // unused transition
  ...
  for(j=1;j<colors;j++){
    ...
    if(rest<colors){
      ...
      m.transition_table[0][j].used = 0; // unused transition
      halts = -1;
    } else {
      rest = rest - colors;      
  ...
  for (i=1; i<states; i++) {  
	...
	m.transition_table[i][j].used = 0; // unused transition
	halts = -1;
      } else {
	rest = rest - colors;
...
int usedInstructions(turing_machine *m){
  int usedIns = 0;
  int i,j;
  for (i=0; i<m->number_states; i++) {  
    for(j=0;j<m->number_colors;j++){      
      usedIns = usedIns + (m->transition_table[i][j].used);
    }
  }
  return usedIns;
}
...
int main(int argn, char* argv[]) {
  ...
  string out;
  int i;
  int usedIns; // number of used instructions
  ...
  while(mpz_cmp(TM,Last)<=0){
	...
	int usedIns = usedInstructions(&machine);
	if(results[out] == 0 || usedIns < results[out]){
	  results[out] = usedIns;
	}
      }
